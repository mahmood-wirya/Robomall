...
export default function ProductCard({ product }) {
  return (
    <Link to={`/products/${product.id}`} className="col-span-3 bg-white p-7 rounded-3xl w-[350px] h-full mt-10 shadow-lg">
      ...
      <p className="tracking-widest text-xl title-font font-medium text-gray-700">{product.price}$</p>
      <div className="border-2 rounded-3xl border-[#2DBCB7] text-[#2DBCB7] p-1">{product.category}</div>
      </div>
    </Link>
  );
}
